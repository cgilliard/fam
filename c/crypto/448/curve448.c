...
static int recode_wnaf(struct smvt_control *control,
	...
	/*
	 * PERF: Could negate scalar if it's large.  But then would need more
	 * cases in the actual code that uses it, all for an expected reduction
	 * of like 1/5 op. Probably not worth it.
	 */
	...
	for (w = 1; w < (C448_SCALAR_BITS - 1) / 16 + 3; w++) {
		...
		while (current & 0xFFFF) {
			...
			/*
			 * Coverity gets confused by the value of pos, thinking
			 * it might be
			 * 32.  This would require current & 0xFFFF to be zero
			 * which isn't possible.  Suppress this false positive,
			 * since adding a check isn't desirable.
			 */
...
void ossl_curve448_base_double_scalarmul_non_secret(
	...
	struct smvt_control
	    control_pre[C448_SCALAR_BITS / (C448_WNAF_FIXED_TABLE_BITS + 1) +
			3];
	...
	if (i > control_pre[0].power) {
		...
		pniels_to_pt(combo, precmp_var[control_var[0].addend >> 1]);
		add_niels_to_pt(
		    combo, ossl_curve448_wnaf_base[control_pre[0].addend >> 1],
		    i);
		contv++;
		contp++;
	} else {
