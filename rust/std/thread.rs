...
use prelude::*;
use sys::{
	alloc, release, thread_create, thread_create_joinable, thread_detach, thread_handle_size,
	thread_join,
};
...
impl JoinHandle {
	pub fn join(&mut self) -> Result<(), Error> {
		...
		} else if unsafe { thread_join(self.handle) } != 0 {
			Err(ErrorKind::ThreadJoin.into())
		} else {
	...
	pub fn detach(&mut self) -> Result<(), Error> {
		if !self.need_detach || unsafe { thread_detach(self.handle) } != 0 {
	...
	F: FnOnce(),
{
	match Box::new(f) {
		Ok(mut b) => {
			unsafe {
				if thread_create(start_thread::<F>, b.as_ptr().raw() as *mut u8) != 0 {
...
where
	F: FnOnce(),
{
	let handle = unsafe { alloc(thread_handle_size()) };
	let jh = JoinHandle {
		handle,
		need_detach: true,
	};
	match Box::new(f) {
		Ok(mut b) => {
			unsafe {
				if thread_create_joinable(jh.handle, start_thread::<F>, b.as_ptr().raw() as *mut u8)
					!= 0
				{
					return Err(ErrorKind::ThreadCreate.into());
				}
