...
pub fn lagrange_vartime(k: &[u64], n: &[u64], max_bitlen: u32, c0: &mut [u64], c1: &mut [u64]) {
	match n.len() {
		4 => {
			let (v0, v1) = unsafe {
				let k_ptr = k.as_ptr() as *const [u64; 4];
				let n_ptr = n.as_ptr() as *const [u64; 4];
				lagrange256_vartime(&*k_ptr, &*n_ptr, max_bitlen)
			};
			unsafe {
				copy_nonoverlapping(v0.as_ptr(), c0.as_mut_ptr(), c0.len());
				copy_nonoverlapping(v1.as_ptr(), c1.as_mut_ptr(), c1.len());
			}
		}
		5 => {
			let (v0, v1) = unsafe {
				let k_ptr = k.as_ptr() as *const [u64; 5];
				let n_ptr = n.as_ptr() as *const [u64; 5];
				lagrange320_vartime(&*k_ptr, &*n_ptr, max_bitlen)
			};
			unsafe {
				copy_nonoverlapping(v0.as_ptr(), c0.as_mut_ptr(), c0.len());
				copy_nonoverlapping(v1.as_ptr(), c1.as_mut_ptr(), c1.len());
		...
		6 => {
			let (v0, v1) = unsafe {
				let k_ptr = k.as_ptr() as *const [u64; 6];
				let n_ptr = n.as_ptr() as *const [u64; 6];
				lagrange384_vartime(&*k_ptr, &*n_ptr, max_bitlen)
			};
			unsafe {
				copy_nonoverlapping(v0.as_ptr(), c0.as_mut_ptr(), c0.len());
				copy_nonoverlapping(v1.as_ptr(), c1.as_mut_ptr(), c1.len());
		...
		7 => {
			let (v0, v1) = unsafe {
				let k_ptr = k.as_ptr() as *const [u64; 7];
				let n_ptr = n.as_ptr() as *const [u64; 7];
				lagrange448_vartime(&*k_ptr, &*n_ptr, max_bitlen)
			};
			unsafe {
				copy_nonoverlapping(v0.as_ptr(), c0.as_mut_ptr(), c0.len());
				copy_nonoverlapping(v1.as_ptr(), c1.as_mut_ptr(), c1.len());
		...
		8 => {
			let (v0, v1) = unsafe {
				let k_ptr = k.as_ptr() as *const [u64; 8];
				let n_ptr = n.as_ptr() as *const [u64; 8];
				lagrange512_vartime(&*k_ptr, &*n_ptr, max_bitlen)
			};
			unsafe {
				copy_nonoverlapping(v0.as_ptr(), c0.as_mut_ptr(), c0.len());
				copy_nonoverlapping(v1.as_ptr(), c1.as_mut_ptr(), c1.len());
		...
		_ => {
			exit!("incorrect lagrange_vartime size");
		}
	}
}
...
#[allow(dead_code)]
pub fn lagrange128_basisconv_vartime(a: &[u64; 2], b: &[u64; 2]) -> (i64, i64, i64, i64, u32) {
