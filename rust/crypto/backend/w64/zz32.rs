...
use super::util32::{addcarry_u32, sgnw, subborrow_u32, umull, umull_add, umull_add2};
...
impl Zu128 {
	...
	pub fn decode(buf: &[u8]) -> Option<Self> {
		if buf.len() != 16 {
			...
			for i in 0..4 {
				x.0[i] =
					u32::from_le_bytes(*<&[u8; 4]>::try_from(&buf[(4 * i)..(4 * i + 4)]).unwrap());
			}
			Some(x)
		}
	}
	...
	pub fn abs(self) -> (u128, u32) {
		...
		(
			(d0 as u128) | ((d1 as u128) << 32) | ((d2 as u128) << 64) | ((d3 as u128) << 96),
			s,
		)
	}
...
impl Zu384 {
	...
	#[inline(always)]
	pub const fn w64le(x0: u64, x1: u64, x2: u64, x3: u64, x4: u64, x5: u64) -> Self {
		Self([
			x0 as u32,
			(x0 >> 32) as u32,
			x1 as u32,
	...
	pub fn trunc_and_rsh_cc(&mut self, b: u32, n: u32) -> (Zu256, Zu128) {
		let n1 = n - 224;
		let n2 = 32 - n1;
		let (d0, cc) = addcarry_u32((self.0[7] >> n1) | (self.0[8] << n2), b, 0);
		let (d1, cc) = addcarry_u32((self.0[8] >> n1) | (self.0[9] << n2), 0, cc);
		let (d2, cc) = addcarry_u32((self.0[9] >> n1) | (self.0[10] << n2), 0, cc);
		let (d3, _) = addcarry_u32((self.0[10] >> n1) | (self.0[11] << n2), 0, cc);
		let mut e = [0u32; 8];
