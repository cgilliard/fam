...
impl GF448 {
	...
	pub fn cswap(a: &mut Self, b: &mut Self, ctl: u32) {
		...
		a.0[3] ^= t;
		b.0[3] ^= t;
		let t = cw & (a.0[4] ^ b.0[4]);
		a.0[4] ^= t;
		b.0[4] ^= t;
		let t = cw & (a.0[5] ^ b.0[5]);
		a.0[5] ^= t;
		b.0[5] ^= t;
		let t = cw & (a.0[6] ^ b.0[6]);
		a.0[6] ^= t;
		b.0[6] ^= t;
	}
	...
	*/
	...
	fn set_decode56_reduce(&mut self, buf: &[u8]) {
		for i in 0..7 {
			let ptr = buf.as_ptr();
			let slice = unsafe { from_raw_parts(ptr.add(i * 8), 8) };
			self.0[i] = from_le_bytes_u64(slice);
		}
	}
	...
	pub fn encode(self) -> [u8; 56] {
		...
		for i in 0..7 {
			let mut v = [0u8; 8];
			to_le_bytes_u64(r.0[i], &mut v);
			unsafe {
				copy_nonoverlapping(v.as_ptr(), d.as_mut_ptr().add(i * 8), 8);
			}
		}
		d
	}
	...
	pub fn set_decode_reduce(&mut self, buf: &[u8]) {
		*self = Self::ZERO;
		...
		while n > 0 {
			...
			let ptr = buf.as_ptr();
			let slice = unsafe { from_raw_parts(ptr.add(n), 8) };
			let t0 = from_le_bytes_u64(slice);

			let slice = unsafe { from_raw_parts(ptr.add(n + 8), 8) };
			let t1 = from_le_bytes_u64(slice);

			let slice = unsafe { from_raw_parts(ptr.add(n + 16), 8) };
			let t2 = from_le_bytes_u64(slice);

			let slice = unsafe { from_raw_parts(ptr.add(n + 24), 4) };
			let t3 = from_le_bytes_u32(slice) as u64;
