...
impl Point {
	...
	*/
	...
	pub fn equals(self, rhs: Self) -> u32 {
		(self.X * rhs.Z).equals(rhs.X * self.Z) & (self.Y * rhs.Z).equals(rhs.Y * self.Z)
	}
	...
	pub fn set_mulgen(&mut self, n: &Scalar) {
		...
		*self = Self::from_affine(&Self::lookup_affine(&PRECOMP_B, sd[14]));
		...
		for i in (0..14).rev() {
			self.set_xdouble(5);
			self.set_add_affine(&Self::lookup_affine(&PRECOMP_B, sd[i]));
			self.set_add_affine(&Self::lookup_affine(&PRECOMP_B75, sd[i + 15]));
			self.set_add_affine(&Self::lookup_affine(&PRECOMP_B150, sd[i + 30]));
			self.set_add_affine(&Self::lookup_affine(&PRECOMP_B225, sd[i + 45]));
			self.set_add_affine(&Self::lookup_affine(&PRECOMP_B300, sd[i + 60]));
			self.set_add_affine(&Self::lookup_affine(&PRECOMP_B375, sd[i + 75]));
		}
	}
	...
	/// THIS FUNCTION IS NOT CONSTANT-TIME; it shall be used only with
	/// public data.
	pub fn verify_helper_vartime(self, R: &Point, s: &Scalar, k: &Scalar) -> bool {
...
const HASH_HEAD: [u8; 8] = [0x53, 0x69, 0x67, 0x45, 0x64, 0x34, 0x34, 0x38];
...
impl PrivateKey {
	...
	pub fn generate() -> Self {
		...
		// TODO: switch to csprng
		crate::sys::safe_rand_bytes(&mut seed);
		Self::from_seed(&seed)
	}
...
impl PublicKey {
	...
	fn verify_inner(self, sig: &[u8], phflag: u8, ctx: &[u8], m: &[u8]) -> bool {
	...
	*/
	...
	fn base_arith() {
		...
		const EPP: [[u8; 57]; 7] = [
			...
			[
				0x8D, 0xCF, 0x19, 0x75, 0x8C, 0x0B, 0x73, 0xED, 0x64, 0x1C, 0xCB, 0xA8, 0x09, 0x19,
				0x96, 0x74, 0x51, 0x20, 0x59, 0x1C, 0xAE, 0xAD, 0xA9, 0x4E, 0x05, 0x5B, 0x00, 0x8E,
				0xE5, 0x41, 0x24, 0xD6, 0x66, 0x2F, 0xE7, 0xC3, 0x38, 0xAA, 0xF3, 0xDF, 0x2B, 0xE7,
				0x43, 0xED, 0x0F, 0x64, 0x19, 0x6B, 0xF4, 0x51, 0x91, 0x79, 0xE6, 0x61, 0x79, 0xA0,
				0x80,
			],
			[
				0x01, 0xB4, 0x4C, 0xBD, 0xF8, 0x95, 0x8B, 0x34, 0x98, 0xA7, 0x4E, 0x9A, 0x4A, 0xCE,
				0x09, 0x23, 0x61, 0x57, 0xC9, 0xB5, 0x3F, 0xC9, 0x8A, 0x4D, 0x3D, 0xA4, 0xA9, 0xE7,
				0x7C, 0xE9, 0x8E, 0xB1, 0x0D, 0x13, 0xE9, 0x4C, 0x48, 0x12, 0x4D, 0xC9, 0xDF, 0xCB,
				0x91, 0xDA, 0x0F, 0x21, 0xC8, 0x17, 0x55, 0xE1, 0xC4, 0xF9, 0xED, 0xE1, 0xD5, 0xCB,
				0x80,
			],
			[
				0xA8, 0x00, 0xE1, 0x95, 0x4A, 0xF3, 0x26, 0x17, 0x7C, 0x3D, 0xF5, 0xC7, 0xBD, 0x40,
				0x05, 0xDA, 0xBC, 0x90, 0xD8, 0xD9, 0x2D, 0x1B, 0x12, 0x39, 0x54, 0x2E, 0xE5, 0x92,
				0x63, 0xC8, 0xF1, 0x29, 0xA1, 0x6C, 0xCF, 0x9F, 0x08, 0x18, 0xCF, 0xA9, 0xA8, 0x60,
				0x14, 0xF7, 0x24, 0xA5, 0x7D, 0xD7, 0x78, 0xA1, 0xFE, 0x97, 0x11, 0xE2, 0x6A, 0x5E,
				0x80,
			],
			[
				0x9D, 0x3E, 0x45, 0xDE, 0x29, 0x44, 0xF6, 0x51, 0xA7, 0xD7, 0x5C, 0x8B, 0x6C, 0x6A,
				0x4B, 0x36, 0x3A, 0xA4, 0x1B, 0x52, 0x14, 0x70, 0xDF, 0xE8, 0x4D, 0x83, 0xF7, 0xFB,
				0x5B, 0x86, 0x88, 0x55, 0x60, 0x05, 0xA5, 0x93, 0x45, 0x0D, 0xD4, 0x96, 0xF5, 0xC2,
				0xBB, 0xC5, 0x90, 0xC3, 0xB3, 0x90, 0xB9, 0x48, 0x34, 0xD0, 0xB9, 0xF8, 0xE8, 0xB4,
				0x80,
			],
			[
				0xDE, 0x38, 0x73, 0xCB, 0x14, 0x84, 0x99, 0x05, 0x43, 0x78, 0xAA, 0xDA, 0x27, 0x6E,
				0xFB, 0x1A, 0x1E, 0x08, 0x2A, 0x20, 0xB7, 0x67, 0x3B, 0x3F, 0x1A, 0x9A, 0x1F, 0x8F,
				0x4E, 0xA7, 0x2C, 0x44, 0x3A, 0xE8, 0x7D, 0xCB, 0x2A, 0xE6, 0xAF, 0x97, 0x53, 0xE5,
				0x3E, 0xC9, 0x16, 0x55, 0x36, 0xC4, 0xB2, 0x17, 0x4A, 0x89, 0x91, 0x81, 0x1A, 0x1B,
				0x00,
			],
			[
				0x71, 0x21, 0xC4, 0xD1, 0x1D, 0xE4, 0x7D, 0x02, 0x3F, 0x67, 0xEF, 0x82, 0x5B, 0x7B,
				0x3F, 0x65, 0xF0, 0xB9, 0xB9, 0x98, 0x29, 0xBE, 0x14, 0x57, 0x10, 0xFC, 0x24, 0x3B,
				0xBA, 0x89, 0xE1, 0x7B, 0xF1, 0xAF, 0xFA, 0xCB, 0x99, 0xBB, 0x10, 0xD2, 0x5F, 0x5E,
				0x30, 0xAB, 0xD0, 0x84, 0x93, 0x39, 0x87, 0xBE, 0x6D, 0xF7, 0xD6, 0x4C, 0x60, 0x57,
				0x80,
			],
		];
	...
	fn mulgen() {
		...
		let enc: [u8; 57] = [
			0x38, 0x7B, 0xF4, 0x5E, 0xF3, 0x62, 0xDC, 0x53, 0x1A, 0x49, 0xF7, 0x93, 0x86, 0x6A,
			0xC5, 0x73, 0x40, 0x72, 0xB5, 0x96, 0x5A, 0x84, 0xB0, 0x57, 0xA8, 0xDC, 0x7E, 0x77,
			0x98, 0x04, 0x51, 0x23, 0xD4, 0xD7, 0x1B, 0x5F, 0x42, 0x24, 0x87, 0x2C, 0x4F, 0xB9,
			0x1D, 0x68, 0xEB, 0x54, 0x16, 0xC1, 0xEF, 0x29, 0x9A, 0x2A, 0x4F, 0xEF, 0x51, 0x6C,
			0x80,
		];
