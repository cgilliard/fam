...
mod tests {
	...
	fn hex_decode_to_slice(input: &str, output: &mut [u8]) -> Result<(), Error> {
		if input.len() % 2 != 0 {
	...
	fn x448_mc() {
		...
		let mut u = k;
		let mut ref1 = [0u8; 56];
		hex_decode_to_slice("3f482c8a9f19b01e6c46ee9711d9dc14fd4bf67af30765c2ae2b846a4d23a8cd0db897086239492caf350b51f833868b9bc2b3bca9cf4113", &mut ref1[..]).unwrap();
		let mut ref1000 = [0u8; 56];
		hex_decode_to_slice("aa3b4749d55b9daf1e5b00288826c467274ce3ebbdd5c17b975e09d4af6c67cf10d087202db88286e2b79fceea3ec353ef54faa26e219f38", &mut ref1000[..]).unwrap();
		for i in 0..1000 {
			let old_k = k;
			k = x448(&u, &k);
			u = old_k;
