...
impl<const SZ: usize> SHA2Small<SZ> {
	...
	const fn compile_time_checks() {
		let _ = &[()][1 - ((SZ == 224 || SZ == 256) as usize)];
	}
	...
	fn update(&mut self, src: &[u8]) {
		...
		while j < src.len() {
			let clen = min(src.len() - j, 64 - ptr);
			unsafe {
				copy_nonoverlapping(src.as_ptr().add(j), self.buf.as_mut_ptr().add(ptr), clen);
			}
			ptr += clen;
			if ptr == 64 {
				self.process();
	...
	fn digest_to(&mut self, dst: &mut [u8]) {
		...
		if self.buf.len() >= 64 {
			...
			if range.end <= self.buf.len() {
				...
				unsafe {
					let slice = from_raw_parts_mut(ptr.add(56), 8);
					to_be_bytes_u64(self.ctr << 3, slice);
				}
			}
		}
		...
		for i in 0..(SZ >> 5) {
			...
			unsafe {
				let slice = from_raw_parts_mut(ptr.add(j), 4);
				to_be_bytes_u32(self.h[i], slice);
			}
		}
	}
	...
	fn reset(&mut self, iv: &[u32; 8]) {
		unsafe {
			copy_nonoverlapping(iv.as_ptr(), self.h.as_mut_ptr(), iv.len());
		}
		self.ctr = 0;
	}
	...
	fn process(&mut self) {
		...
		for i in 0..16 {
			...
			unsafe {
				let slice = from_raw_parts_mut(ptr.add(i << 2), 4);
				w[i] = from_be_bytes_u32(slice);
			}
		}
		for i in 16..64 {
			w[i] = ssig1(w[i - 2])
...
impl<const SZ: usize> SHA2Big<SZ> {
	...
	fn update(&mut self, src: &[u8]) {
		...
		while j < src.len() {
			let clen = min(src.len() - j, 128 - ptr);
			unsafe {
				copy_nonoverlapping(src.as_ptr().add(j), self.buf.as_mut_ptr().add(ptr), clen);
			}
			ptr += clen;
	...
	fn digest_to(&mut self, dst: &mut [u8]) {
		...
		let ptr = self.buf.as_mut_ptr();
		unsafe {
			let slice = from_raw_parts_mut(ptr.add(112), size_of::<u128>());
			to_be_bytes_u128(self.ctr << 3, slice);
		}
		self.process();
		for i in 0..(SZ >> 6) {
			let j = i << 3;
			let ptr = dst.as_mut_ptr();
			unsafe {
				let slice = from_raw_parts_mut(ptr.add(j), 8);
				to_be_bytes_u64(self.h[i], slice);
			}
		}
		if (SZ & 63) != 0 {
			let j = (SZ >> 3) & !7usize;
	...
	fn process(&mut self) {
		...
		for i in 0..16 {
			let ptr = self.buf.as_mut_ptr();
			unsafe {
				let slice = from_raw_parts_mut(ptr.add(i << 3), 8);
				w[i] = from_be_bytes_u64(slice);
			}
		}
		for i in 16..80 {
			w[i] = ssig1(w[i - 2])
...
mod tests {
	...
	fn inner_sha224(src: &[u8], hexdst: &str) {
		...
		assert!(Sha224::hash(src) == &dst[0..dst.len()]);
	...
	fn inner_sha256(src: &[u8], hexdst: &str) {
		...
		assert!(Sha256::hash(src) == &dst[0..dst.len()]);
	...
	fn inner_sha384(src: &[u8], hexdst: &str) {
		...
		assert!(Sha384::hash(src) == &dst[0..dst.len()]);
	...
	fn inner_sha512(src: &[u8], hexdst: &str) {
		...
		assert!(Sha512::hash(src) == &dst[0..dst.len()]);
	...
	fn inner_sha512_224(src: &[u8], hexdst: &str) {
		...
		assert!(Sha512_224::hash(src) == &dst[0..dst.len()]);
	...
	fn inner_sha512_256(src: &[u8], hexdst: &str) {
		...
		assert!(Sha512_256::hash(src) == &dst[0..dst.len()]);
