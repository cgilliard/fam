...
impl<const SZ: usize> SHAKE<SZ> {
	...
	pub fn extract(&mut self, dst: &mut [u8]) {
		...
		while i < dst.len() {
			...
			let clen = min(dst.len() - i, Self::RATE - ptr);
			for _ in 0..clen {
...
mod tests {
	...
	fn inner_shake<const SZ: usize>(kat: &[&str]) {
		for i in 0..(kat.len() >> 1) {
			let src = hex_decode(&kat[2 * i]).unwrap();
			let dst = hex_decode(&kat[2 * i + 1]).unwrap();
			let r = &mut [0u8; 512][..dst.len()];

			// Test 1: inject and extract in one go.
			let mut sh = SHAKE::<SZ>::new();
			sh.inject(&src[0..src.len()]);
			sh.flip();
			sh.extract(r);
			assert!(r == &dst[0..dst.len()]);
	...
	fn sha3() {
		for i in 0..(KAT_SHA3.len() >> 1) {
			let m = hex_decode(KAT_SHA3[(i << 1) + 0]).unwrap();
			let d = hex_decode(KAT_SHA3[(i << 1) + 1]).unwrap();
			match d.len() {
				28 => {
					sha3_224(&m[0..m.len()], &d[0..d.len()]);
				}
				32 => {
					sha3_256(&m[0..m.len()], &d[0..d.len()]);
				}
				48 => {
					sha3_384(&m[0..m.len()], &d[0..d.len()]);
				}
				64 => {
					sha3_512(&m[0..m.len()], &d[0..d.len()]);
				}
				_ => {
					assert!(false);
				}
