#[macro_export]
macro_rules! define_gfgen {
	($typename:ident, $fieldparams:ident, $squarespec:expr) => {
		// We define a sub-module so that the 'use' clauses do not spill over
		// the caller.
		pub use self::scalarmod::$typename;
		mod scalarmod {

			use super::$fieldparams;
			use crate::crypto::gf448::{
			...
			impl $typename {
				...
				pub const MODULUS: [u64; 7] = Self::make_modulus();
				...
				pub fn set_mul_small(&mut self, k: u32) {
					...
					for _ in 0..2 {
						...
						let mut cc = 0;
						for i in 0..Self::N {
							let (d, ee) = addcarry_u64(self.0[i], m & Self::MODULUS[i], cc);
							self.0[i] = d;
							cc = ee;
						}
				...
				fn set_montylin(&mut self, u: &Self, v: &Self, f: u64, g: u64) {
					...
					let mut cc2 = 0;
					for i in 0..Self::N {
						let (d, ee) = subborrow_u64(self.0[i], Self::MODULUS[i], cc2);
						self.0[i] = d;
						cc2 = ee;
					}
				...
				pub fn split_vartime(self) -> ([u8; Self::SPLIT_LEN], [u8; Self::SPLIT_LEN]) {
				...
				pub fn set_decode_reduce(&mut self, buf: &[u8]) {
					...
					while j >= CHUNK_LEN {
						...
						let mut cc = 0;
						for i in 0..Self::N {
							let (w, ee) = addcarry_u64(self.0[i], x.0[i], cc);
							self.0[i] = w;
							cc = ee;
						}
				...
				const fn subm(a: Self) -> (Self, u64) {
					const fn subm_inner(a: $typename, cc: u64, i: usize) -> ($typename, u64) {
						if i == a.0.len() {
							...
							let (d, cc) = if i < a.0.len() {
								$typename::sbb(a.0[i], $typename::MODULUS[i], cc)
							} else {
								(0, 0)
							};
							let mut aa = a;
				...
				const fn addm_cond(a: $typename, mm: u64) -> $typename {
					const fn addm_cond_inner(
						...
						if i == a.0.len() {
							...
							let (d, cc) = if i < a.0.len() {
								$typename::adc(a.0[i], $typename::MODULUS[i] & mm, cc)
							} else {
								(0, 0)
							};
							let mut aa = a;
				...
				const fn top_word_index() -> usize {
					const fn top_word_index_inner(j: usize) -> usize {
						if j < $fieldparams::MODULUS.len() && $fieldparams::MODULUS[j] != 0 {
							j
						} else {
				...
				const fn make_modulus() -> [u64; Self::N] {
					const fn make_modulus_inner(
						...
						if j == $typename::N {
							...
							let mut dd = d;
							if j < dd.len() && j < $fieldparams::MODULUS.len() {
				...
				const fn const_neg(a: Self) -> Self {
					const fn const_neg_inner(
						...
						if j == $typename::N {
							...
							let mut dd = d;
							if j < $typename::MODULUS.len() && j < a.0.len() && j < dd.0.len() {
					...
					const fn mmul1(
						...
						const fn mmul1_inner(
							...
							if i == d.0.len() {
								...
								let (z, cc1) = if i < a.0.len() && i < d.0.len() {
									umaal(a.0[i], bj, d.0[i], cc1)
								} else {
									(0, 0)
								};
								let (z, cc2) = if i < $typename::MODULUS.len() {
									umaal($typename::MODULUS[i], fm, z, cc2)
								} else {
									(0, 0)
								};
								let mut dd = d;
			...
			impl ops::Add<$typename> for $typename {
			...
			impl ops::AddAssign<$typename> for $typename {
				#[inline(always)]
			...
			impl ops::Div<$typename> for $typename {
				type Output = $typename;
			...
			impl ops::DivAssign<$typename> for $typename {
				#[inline(always)]
			...
			impl ops::Mul<$typename> for $typename {
				type Output = $typename;
			...
			impl ops::MulAssign<$typename> for $typename {
				#[inline(always)]
			...
			impl ops::Sub<$typename> for $typename {
				type Output = $typename;
			...
			impl ops::SubAssign<$typename> for $typename {
				#[inline(always)]
...
#[macro_export]
macro_rules! define_gfgen_tests {
	($typename:ident, $nqr:expr, $submod:ident) => {
		#[cfg(test)]
		mod $submod {
			...
			*/
			...
			fn field_ops() {
				...
				assert!(
					$typename::decode_reduce(&va).equals($typename::decode_reduce(&vb))
						== 0xFFFFFFFF
				);
